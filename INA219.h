...
public:
  ...
  uint16_t getMiliVoltage(void) {
    ...
    return (value >> 2) * 2.0f * _calibVal;             // LSB = 4mV = 0.004V, Сдвигаем значение до 12 бит и умножаем
  }

  // Чтение напряжения
  float getVoltage(void) {
    return ((float)getMiliVoltage() *0.001f);             // LSB = 4mV = 0.004V, Сдвигаем значение до 12 бит и умножаем
  }
  ...
  float getMiliCurrent(void) {
    ...
    return (value * _current_lsb * 1000);                        // LSB рассчитывается на основе макс. ожидаемого тока, умножаем и возвращаем
  }
...
//private:
  ...
  const bool _v_max = 0x00;           // Макс. ожидаемый напря
  ...
  void calibrate(void) {
    ...
    uint16_t cfg_register = readRegister(INA219_CFG_REG_ADDR) & ~(0b0111 << 11);  // Читаем регистр калибровки, сбрасываем биты диапазона напряжения шунта
    ...
    if (!_v_max) cfg_register |= 0b0 << 13;                     //  - выбираем 16V
